...
<\/script>
    ...
    // POST /bblm-foto/clear — hapus semua data foto (sebelum :id match)
    if (path === "/bblm-foto/clear" && method === "POST") {
      const list = await getJson("bblm-foto", []);
      await Promise.all(list.map(it => deleteKey("bblm-foto-photo-" + it.id).catch(() => {})));
      await setJson("bblm-foto", []);
      await setJson("bblm-foto-downloaded", []);
      return send({ success: true });
    }

    // POST /bblm-foto/bulk-delete — hapus beberapa item sekaligus
    if (path === "/bblm-foto/bulk-delete" && method === "POST") {
      const { ids } = body;
      if (!Array.isArray(ids)) return send({ success: false, message: "ids harus array" }, 400);
      let list = await getJson("bblm-foto", []);
      const toDelete = new Set(ids);
      list = list.filter(it => !toDelete.has(it.id));
      await Promise.all([...toDelete].map(id => deleteKey("bblm-foto-photo-" + id).catch(() => {})));
      await setJson("bblm-foto", list);
      let downloaded = await getJson("bblm-foto-downloaded", []);
      downloaded = downloaded.filter(id => !toDelete.has(id));
      await setJson("bblm-foto-downloaded", downloaded);
      return send({ success: true, deleted: ids.length });
    }
    ...
    // GET /bblm-foto — daftar metadata + downloadedIds
    if (path === "/bblm-foto" && method === "GET") {
      const [list, downloadedIds] = await Promise.all([
        getJson("bblm-foto", []),
        getJson("bblm-foto-downloaded", [])
      ]);
      return send({ success: true, count: list.length, items: list, downloadedIds });
    }

    // POST /bblm-foto — tambah item baru
    if (path === "/bblm-foto" && method === "POST") {
      const { randomUUID } = await import("crypto");
      const { prodCd, namaBarang, category, username, photo } = body;
      const list = await getJson("bblm-foto", []);
      const id = randomUUID();
      const item = { id, prodCd: prodCd || "", namaBarang: namaBarang || "", category: category || "", username: username || "", createdAt: new Date().toISOString(), hasPhoto: !!photo };
      if (photo) await setJson("bblm-foto-photo-" + id, photo);
      list.unshift(item);
      await setJson("bblm-foto", list);
      return send({ success: true, id });
    }

    // /bblm-foto/:id/photo — GET atau POST foto
    const bblmFotoPhotoMatch = path.match(/^\/bblm-foto\/([^/]+)\/photo$/);
    if (bblmFotoPhotoMatch) {
      const id = bblmFotoPhotoMatch[1];
      if (method === "GET") {
        const photo = await getJson("bblm-foto-photo-" + id, null);
        if (!photo) return send({ error: "Photo not found" }, 404);
      ...
      if (method === "POST") {
        const { photo: fotoBase64 } = body;
        if (!fotoBase64) return send({ success: false, message: "photo wajib diisi" }, 400);
        await setJson("bblm-foto-photo-" + id, fotoBase64);
        const list = await getJson("bblm-foto", []);
    ...
    const bblmFotoPatchMatch = path.match(/^\/bblm-foto\/([^/]+)$/);
    if (bblmFotoPatchMatch && method === "PATCH") {
      const id = bblmFotoPatchMatch[1];
      const list = await getJson("bblm-foto", []);
      const idx = list.findIndex(it => it.id === id);
      if (idx < 0) return send({ success: false, message: "Item tidak ditemukan" }, 404);
      const { prodCd, namaBarang, category } = body;
      if (prodCd !== undefined) list[idx].prodCd = prodCd;
      if (namaBarang !== undefined) list[idx].namaBarang = namaBarang;
    ...
    // DELETE /bblm-foto/:id
    if (bblmFotoPatchMatch && method === "DELETE") {
      const id = bblmFotoPatchMatch[1];
      let list = await getJson("bblm-foto", []);
      list = list.filter(it => it.id !== id);
      await setJson("bblm-foto", list);
      await deleteKey("bblm-foto-photo-" + id).catch(() => {});
      return send({ success: true });
    }
