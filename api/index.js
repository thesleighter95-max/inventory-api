...
export default async function handler(req) {
  ...
  try {
    ...
    if (path === "/bblm" && method === "GET") {
      return json(await getJson("bblm", { hasData: false, gradeNames: [], products: [], totalProducts: 0, updatedAt: null, updatedBy: "", sourceLabel: "" }));
    }
    ...
    if (path === "/activity-log" && method === "GET") {
      ...
      return json({ success: true, logs: logs.slice(0, maxLimit) });
    }
    ...
    if (path === "/login" && method === "POST") {
      ...
      if (!user) return json({ success: false, notFound: true, message: "User tidak ditemukan di server" }, 404);
    ...
    if (path === "/backup" && method === "GET") {
      ...
      return json({ success: true, exportedAt: new Date().toISOString(), data: { users, bblm, "activity-logs": logs, "product-requests": requests, maintenance, "bblm-status": bblmStatus } });
    }
    ...
    if (path.startsWith("/price-history/") && method === "GET") {
      ...
      const [individual, prev] = await Promise.all([getJson(`price-history:${barcode}`, null), getJson("price-snapshot-prev", { date: null, prices: {} })]);
      const prevPrice = prev.prices?.[barcode];
      return json({ success: true, data: prevPrice != null ? { price: prevPrice, date: prev.date } : individual });
    }
    ...
    if (path === "/sync-prices" && method === "GET") {
      const [current, prev] = await Promise.all([getJson("price-snapshot-current", { date: null, prices: {} }), getJson("price-snapshot-prev", { date: null, prices: {} })]);
      return json({ success: true, current: { date: current.date, count: Object.keys(current.prices || {}).length }, prev: { date: prev.date, count: Object.keys(prev.prices || {}).length, prices: prev.prices || {} } });
    }
